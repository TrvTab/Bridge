...
function MarkerList(props) {
  ...
  const handleRemove = (key) => {
    setMarkerItems((markerItems) =>
      markerItems.filter((item) => item.key !== key)
    );
  };
        ...
        <li list-style="none" key={title}>
          <Row>
            <Marker
              ...
              colour={colour}
              time={time}
              onMarkerClicked={(key) => handleMarkerClicked(key)}
            ></Marker>
  ...
  const handleGoToMarker = (key, request) => {
    let foundItem = markerItems.find((item) => item.key === key).props.children
  ...
  useEffect(() => {
    if (goToMarkerDest) handleGoToMarker(goToMarkerDest, "goToMarker");
  }, [goToMarkerDest]);
  ...
  const validateAddMarker = (commandData) => {
    ...
    let duplicate = markerItems.find(
      (element) => commandData.name === element.key
    );
    if (duplicate) {
  ...
  useEffect(() => {
    if (props.commandInformation.request.includes("loop")) return;
      ...
      if (validateAddMarker(props.commandInformation)) {
        submitMarker(
          props.commandInformation.name,
          "colour",
          convertToMinutes(props.commandInformation.firstTimeStamp)
        );
      }
      ...
      if (validateMarkerPresent(props.commandInformation)) {
        handleGoToMarker(
          props.commandInformation.name,
          props.commandInformation.request
        );
      }
    }
    ...
    <div style={{ height: 70, width: 300, float: "right" }}>
      <Container>
        {!showForm && (
          <ul>
            {markerItems}
            <Button className="controlButton" onClick={addMarker}>
              Add Marker
            </Button>
          </ul>
        )}
